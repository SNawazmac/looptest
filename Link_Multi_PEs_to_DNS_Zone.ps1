...
[Parameter(Mandatory=$true)][string]$Private_DNS_Zone_resource_group_name,       #Enter the primary resourcegroup name where the Private DNS Zone is existing
...
[Parameter(Mandatory=$true)][array]$secondary_private_endpoint_names,   #Enter the secondary private endpoint name
...
$SecondaryPEs = $secondary_private_endpoint_names.split(",")

foreach($secondary_private_endpoint_name in $secondaryPEs)
{
